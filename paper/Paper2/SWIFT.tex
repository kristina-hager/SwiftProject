...
\begin{abstract}
	...
	We propose a more efficient and safe means of data retrieval which is to automatically ferry data back to the surface using WiFi Direct enabled devices. 
...
Many types of scientists such as geologists, hydro geologists, and climatologists conduct research in the interior of caves. 
...
For these entire reasons cave researchers are interested in mechanisms that would make their data easier to access.
...
% A cave environment is distinctly different from a common surface environment in that there are significantly more obstructions between devices. 
...
\begin{figure*}[t]
\includegraphics[width=\textwidth]{AndroidWifiAPI}
\caption{WiFi Direct API function and intent call flow.}
\label{fig:androidwifiapi}
\end{figure*}

The application invokes the WiFi peer to peer manager "connect()" function to establish a connection to a selected device.
...
Figure~\ref{fig:androidwifiapi} shows the complete call flow for the Android WiFi Direct API peer to peer manager calls, listeners, and intents.
...
%Leaving this line out of the final paper is fine, but the comment is fine to leave too in case I get to the 'todo'
...
WiFi Direct operates on the same 2.4 Ghz band as most 802.11 based WiFi deployments \cite{wifiwhitepaper}.
...
\item Endpoint
\end{enumerate}
A device's role is determined by whether a single upstream, a single downstream, or both an upstream and downstream device is set.
...
Once the initial file was received, then subsequent transfers went well.
...
%Possibly see if there is a way to make this sentence more clear?
...
A communication system to coordinate re-supplies, logistics, and, in the worst case, a rescue operation is almost a requirement for an expedition of this magnitude.
...
%and some similarly large and long horizontal passages 
\cite{krub_it}.
...
% todo: consider trying to reword the below to be a shorter sentence

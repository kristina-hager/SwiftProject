...
A scientist will not know that her data logger has malfunctioned until she is able to visit the data logger.
\begin{figure*}[t]
...
The second phase of the project was to build an Android application that would utilize Wifi Direct to do an automated transfer of a datalogger datafile across multiple mobile devices acting as a data ferry. 
This project leveraged some of the Wifi Direct data transfer code from the range software, but implemented a state machine to allow the application to execute in "Operate Mode" where data hops from device to device. 
...
TODO: add some Krubera photos from online - very cool pics available
...
Power conservation would be fairly important in a cave environment as access to a power outlet would be unlikely.
During this project no effort was placed on looking at power consumption in general, or any methods to optimize or minimize the time that wireless would need to be active.
This application would be particularly challenging to implement downtime for wireless since upstream devices currently don't have any mechanism to know when a downstream device might try to make a connection.
...
Future work could be considered to refactor improvements in wifi direct middleware 

As noted in the results section, timing was a key challenge due to the highly asynchronous nature of the application.
There is additional work that could be spent implementing a better mechanism to determine that a device in any phase is in the desired state and ready before operations are executed.
In the prototype application, delays were injected to give time for devices to completed known longer running tasks, but a better mechanism could be to look at some sort of handshake, acknownledge, and retry design.
Handling endpoints is another area that was not explored during this project.
Both interfacing to actual dataloggers as well as interfacing to the broader Internet for data uploads once the application has successfully ferried data from the cave.

Environmental issues such as dust and possibly flooding are both areas that might need to be considered in some or all cases for a practical implementation.
